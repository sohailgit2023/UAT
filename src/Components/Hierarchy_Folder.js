...
const Hierarchy_Folder = ({ testCycle, testCycleName, fileType, icon }) => {
    ...
    return (
        <>
            <div className={`w-full flex items-center gap-0.5 mt-2 lg:mt-1 ml-1 sm:ml-6 mr-1 sm:mr-6 md:mr-6 lg:mr-10 hover:bg-cyan-200 hover:text-black ${location.pathname===`/ExecuteTestRun/${testCycle.id}` && 'bg-green-300 hover:bg-green-300'}`} title={`${testCycle.pid} ${testCycle.name}`} onClick={()=>testCycle.pid.split('-')[0]=='TS' && handel_TestCycleID_Or_TestSuiteId(testCycle.id, testCycle.name, testCycleName)}>
                {expand ? <MdOutlineArrowDropDown className={`text-base sm:text-lg md:text-xl cursor-pointer ${hideExpandIcon && 'hidden'}`} onClick={handelTestSuitClose} /> : <MdArrowRight className={`text-base sm:text-lg md:text-xl cursor-pointer ${hideExpandIcon && 'invisible'}`} onClick={handelTestSuitOpen} />}
                <div className="flex items-center gap-1 lg:gap-2 truncate cursor-pointer">
                    {icon=='test-Cycles' ? <PiWaveSineBold className=" text-sm sm:text-base lg:text-lg bg-blue-500 text-white p-0.5" /> :<BiNotepad className=" text-sm sm:text-base lg:text-lg text-yellow-500 text-white" />}
                    <span className="text-xs sm:text-sm lg:text-sm truncate">{testCycle.name}</span>
                </div>
            </div>
            {
                expand && 
                    testCycle['test-cycles'].map((testCycleElement, testCycleIndex) => (
            ...
            {
                expand && testCycle['test-suites'].map((testSuiteElement, testSuiteIndex)=>(
